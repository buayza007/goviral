...
import { Sparkles, Trophy, Flame, MessageCircle, Share2, Heart, Eye, ExternalLink, Play, FileText } from "lucide-react";
import { PageSearchForm } from "@/components/dashboard/page-search-form";
...
function PostCard({ content }: { content: ContentItem }) {
  ...
  return (
    ...
    >
      <Card className="overflow-hidden bg-slate-800/50 border-slate-700 hover:border-emerald-500/50 transition-all group hover:shadow-xl h-full flex flex-col">
        {/* Media */}
        ...
        <div className="p-4 flex-1 flex flex-col">
          ...
          {content.pageName && (
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-emerald-400 font-medium truncate">{content.pageName}</span>
            </div>
          )}
          ...
          {content.url && (
            <a
              ...
              className="mt-3 flex items-center justify-center gap-2 py-2.5 rounded-lg bg-emerald-500/20 text-emerald-400 text-sm font-medium hover:bg-emerald-500/30 transition-colors"
            >
    ...
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto space-y-8 px-4">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center pt-4">
          ...
          <p className="text-gray-400 mt-2">ดึงโพสต์จากเพจ Facebook โดยตรง ไม่ต้องใช้ Cookie</p>
        </motion.div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">
                    จากเพจ: <span className="text-emerald-400">{searchResult.pageUrls.length} เพจ</span>
                  </p>
                </div>
              </div>
        ...
        {searchResult && (!searchResult.contents || searchResult.contents.length === 0) && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16">
            ...
            <p className="text-gray-400">ลองใส่ URL เพจใหม่</p>
          </motion.div>
        )}
      </div>
    </div>
